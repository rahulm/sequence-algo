...
class SequencePlayer:
    ...
    def __init__(
        ...
        self.playerToTeamMap = {
            (player + 1): ((player % self.numTeams) + 1)
        ...
        self.board = [
            [0 for i in range(self.boardLen)] for j in range(self.boardLen)
        ]
        ...
        if teamColors is None:
            self.teamColorNames = {
                t: str(t) for t in range(1, self.numTeams + 1)
            }
            ...
            self.teamColorNames = {
                (t + 1): tcolor.lower() for t, tcolor in enumerate(teamColors)
            }
            self.teamColorAscii = {
                t: ASCII_COLOR_CODES[tcolor]
                for t, tcolor in self.teamColorNames.items()
            }
    ...
    def printBoard(self):
        for row in self.board:
            for val in row:
                head = (
                    ""
                    if ((val == "free") or (val == 0))
                    else self.teamColorAscii[abs(val)]
                )
                foot = (
                    ""
                    if ((val == "free") or (val == 0))
                    else self.teamColorAscii["end"]
                )
    ...
    def printSequence(self, seq):
        emptyBoard = [
            [0 for r in range(self.boardLen)] for c in range(self.boardLen)
        ]
        for (r, c) in seq:
            emptyBoard[r][c] = "X"
    ...
    def initBotHand(self):
        ...
        while True:
            ...
            if len(cards) != self.numCardsPerHand:
                print(
                    "Wrong number of cards. Need {} cards total. Retrying.".format(
                        self.numCardsPerHand
                    )
                )
                continue
    ...
    def play(self):
        ...
        # markers
        markers = {"retry": False, "twoEyedJack": False, "oneEyedJack": False}
    ...
    def recordPlayerTurn(self, playerToMove, teamToMove, markers):
        if not (
            markers["retry"] or markers["oneEyedJack"] or markers["twoEyedJack"]
        ):
            print(
                "Player {}'s (team {}) turn.".format(
                    playerToMove, self.teamColorNames[teamToMove]
                )
            )
        ...
        if card == "pass":
            ...
            allowedVals = {
                team
                for team in range(1, self.numTeams + 1)
                if (teamToMove != team)
            }
            options = [
                (r, c)
            ...
            options = [
                ...
                for c in range(self.boardLen)
                if (
                    (BOARD_CARD_LAYOUT[r][c] != "free")
                    and (self.board[r][c] in allowedVals)
                )
            ]
            ...
            if len(options) == 0:
                ...
                for optionInd, (r, c) in enumerate(options):
                    print(
                        "    {}  -  row={} col={}".format(optionInd + 1, r, c)
                    )
                ...
                self.board[optionR][optionC] = (
                    0 if markers["oneEyedJack"] else teamToMove
                )
    ...
    def calculateHeuristicScoreOnSegment(
        self, teamId, segmentLength, rStart, cStart, rSlope, cSlope
    ):
        ...
        for t in range(1, self.numTeams + 1):
            ...
            if t == teamId:
                weight = pow(
                    (self.teamScores[t] + 1) / self.numSequencesToWin, 3
                )
                ts = pow(
                    numPawns
                    + (1 if (numPawns == self.numPawnsInASequence) else 0),
                    2,
                )
                ...
                weight = pow(
                    (
                        ...
                        + (
                            2
                            if (
                                (numPawns == (self.numPawnsInASequence - 1))
                                and (numBotPawns == 0)
                                and (
                                    self.teamScores[t]
                                    == (self.numSequencesToWin - 1)
                                )
                            )
                            else 1
                        )
    ...
    def calculateHeuristicScore(self, teamId):
        ...
        for r in range(-self.numPawnsInASequence + 1, self.boardLen):
            for c in range(self.boardLen):
                vals.append(
                    self.calculateHeuristicScoreOnSegment(
                        teamId, self.numPawnsInASequence, r, c, 1, 0
                    )
                )
        ...
        for r in range(self.boardLen):
            for c in range(-self.numPawnsInASequence + 1, self.boardLen):
                vals.append(
                    self.calculateHeuristicScoreOnSegment(
                        teamId, self.numPawnsInASequence, r, c, 0, 1
                    )
                )
        ...
        for r in range(-self.numPawnsInASequence + 1, self.boardLen):
            for c in range(-self.numPawnsInASequence + 1, self.boardLen):
                vals.append(
                    self.calculateHeuristicScoreOnSegment(
                        teamId, self.numPawnsInASequence, r, c, 1, 1
                    )
                )
    ...
    def playBotTurn(self, botPlayerId, botTeamId):
        ...
        for cardInd in range(len(self.botHand)):
            ...
            # get open slots
            slots = sum(
                1 for (r, c) in CARD_LOCS[card] if (self.board[r][c] == 0)
            )

            # replace card if no open slots found
            if slots == 0:
                print(
                    "Discarding {}. Please pick up a replacement.".format(card)
                )
                while True:
        ...
        bestAction = {"card": None, "location": None, "score": float("-inf")}
        # bestAction contains:
        ...
        killerMoveNeeded = self.teamScores[botTeamId] == (
            self.numSequencesToWin - 1
        )
        ...
        for card in self.botHand:
            ...
            if card in ONE_EYED_JACKS:
                isRemoval = True
                allowedVals = {
                    t for t in range(1, self.numTeams + 1) if (t != botTeamId)
                }
                locationOptions = [
                    (r, c)
                    for r in range(self.boardLen)
        ...
        if bestAction["card"] is None:
            ...
            # print action
            print(
                ">>> Played {} at row={} col={} <<<".format(
                    bestCard, bestLocR, bestLocC
                )
            )
            # if it was a removal (replacement with empty=0), notify
            if valToPlace == 0:
                print(
                    ">>> Removed team {}'s pawn from {} <<<".format(
                        self.teamColorNames[prevTeamVal],
                        BOARD_CARD_LAYOUT[bestLocR][bestLocC],
                    )
                )
            # if it was a two eyed jack, notify
            if bestCard in TWO_EYED_JACKS:
                print(
                    ">>> Placed pawn on {} space <<<".format(
                        BOARD_CARD_LAYOUT[bestLocR][bestLocC]
                    )
                )
